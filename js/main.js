...
document.addEventListener("DOMContentLoaded", function () {
	...
	const observer = new IntersectionObserver(
	  (entries) => {
		entries.forEach((entry) => {
		  ...
		  if (entry.isIntersecting) {
			// Add the animate class first
			bar.classList.add("animate");
			// Then set the width to trigger the transition
			setTimeout(() => {
			  bar.style.setProperty("--progress-width", width);
			}, 10); // Small delay to ensure the class is applied
		  } else {
			// Reset when the bar leaves the view
	  ...
	  },
	  {
		...
		rootMargin: "0px 0px -50px 0px", // Adjust the root margin to delay the trigger
	  }
	);
